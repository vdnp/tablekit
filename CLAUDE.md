...
> Last updated: 2026-07-07 (docs set complete: package READMEs, guides, generated API reference, doc-check tooling)
...
- **Doc QC is home-grown, offline** (`docs/scripts/check-code-blocks.mjs`, `check-links.mjs`) instead of remark/markdown-link-check — no network, deterministic, and the code checker reuses the repo's own TypeScript. Only fenced blocks whose info string contains `check` are compiled (against package `src` via tsconfig `paths`, so no build needed); RN snippets are left unchecked because `react-native` doesn't resolve from the repo root (same Flow-source limitation as RN component tests).
